...
// Not:
// Bu kontrolcü downwash/bozucu kuvvet topic'ine abone değildir.
// Downwash kuvveti kontrolcüye feedforward olarak verilmez.
// LESO yalnızca ölçülen hız cevabından toplam bozucu ivme etkisini kestirir.
      ...
      m_filtered_cmd_x(0.0), m_filtered_cmd_y(0.0), m_filtered_cmd_z(0.0)
{
    ...
    // LESO Kazanç Parametreleri (Bant genişliği rad/s cinsinden)
...
double MidLevelController::computeThrust() {
    ...
    // ------------------------------------------------------------------------
    // NOMİNAL MODEL DESTEKLİ GENİŞLETİLMİŞ DURUM GÖZLEMCİSİ (LESO) ADIMI
    // ------------------------------------------------------------------------
    ...
    // Nominal giriş kazancı. Downwash modeli burada kullanılmaz.
    ...
    // ------------------------------------------------------------------------
    // KONTROL YASASI (NOMİNAL MODEL DESTEKLİ LESO TABANLI BOZUCU BASTIRMA)
    // ------------------------------------------------------------------------
    // PID çıktısı kuvvet (N) cinsinden olduğu için ivme birimine indirgenir (1/m).
    // Kestirilen toplam bozucu ivme döngüden düşülür.
    // Burada downwash kuvveti feedforward olarak kullanılmaz.
...
void MidLevelController::publishControlSignals(double thrust, const geometry_msgs::Vector3& ref_angles) {
    ...
    // Kestirilen toplam bozucu ivme değerleri (m/s^2) ROS'a basılıyor.
    // Bu değer fiziksel downwash kuvveti değil, LESO'nun tahmin ettiği lumped disturbance etkisidir.
