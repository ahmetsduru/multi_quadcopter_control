...
for i = 1:num_drones
    ...
    if isfile(filename)
        data = readtable(filename, 'VariableNamingRule', 'preserve');
        h = plot3(data{:, "a_pos.x"}, data{:, "a_pos.y"}, data{:, "a_pos.z"}, '-', 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', sprintf('Q %d', i));
...
%% === 17. ROTOR RPM GRAFİKLERİ (HER DRONE İÇİN AYRI) ===
fig = figure;
set(fig, 'Position', [100, 100, 1200, 800]); % Pencere boyutu

% Motorlar için özel renk paleti (Kırmızı, Mavi, Yeşil, Siyah)
motor_colors = [
    0.8500 0.3250 0.0980; % M1 (Turuncu-Kırmızımsı)
    0.0000 0.4470 0.7410; % M2 (Mavi)
    0.4660 0.6740 0.1880; % M3 (Yeşil)
    0.2000 0.2000 0.2000  % M4 (Koyu Gri/Siyah)
];

for i = 1:num_drones
    subplot(3, 2, i); % 5 drone için 3x2 yerleşim
    hold on; box on;
    ...
    if isfile(filename)
        ...
        % RPM verisinin olup olmadığını kontrol et
        if ismember('rpm_1', data.Properties.VariableNames)
            time = data{:, "time"} - data{1, "time"};
            
            % 4 Motoru Çizdir
            p1 = plot(time, data{:, "rpm_1"}, 'Color', motor_colors(1,:), 'LineWidth', 1.5, 'DisplayName', 'M1');
            p2 = plot(time, data{:, "rpm_2"}, 'Color', motor_colors(2,:), 'LineWidth', 1.5, 'DisplayName', 'M2');
            p3 = plot(time, data{:, "rpm_3"}, 'Color', motor_colors(3,:), 'LineWidth', 1.5, 'DisplayName', 'M3');
            p4 = plot(time, data{:, "rpm_4"}, 'Color', motor_colors(4,:), 'LineWidth', 1.5, 'DisplayName', 'M4');
            
            has_rpm_data = true;
        else
            text(30, 0.5, 'RPM Data Not Found', 'HorizontalAlignment', 'center', ...
    ...
    % --- Görsel Ayarlar ---
    if has_rpm_data
        % Y eksenini veriye göre ayarla ama biraz boşluk bırak
        max_rpm = max([max(data{:, "rpm_1"}), max(data{:, "rpm_2"}), ...
                       max(data{:, "rpm_3"}), max(data{:, "rpm_4"})]);
        if max_rpm == 0, max_rpm = 1000; end % Veri yoksa patlamasın
        ylim([0, max_rpm * 1.2]);
    end
    ...
    % Arka Plan Zonları
    pt1 = patch(t_zone1, [yl(1) yl(1) yl(2) yl(2)], c_zone1, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    pt2 = patch(t_zone2, [yl(1) yl(1) yl(2) yl(2)], c_zone2, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    pt3 = patch(t_zone3, [yl(1) yl(1) yl(2) yl(2)], c_zone3, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    pt4 = patch(t_zone4, [yl(1) yl(1) yl(2) yl(2)], c_zone4, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    uistack([pt1 pt2 pt3 pt4], 'bottom'); 

    % Etiketler
    title(sprintf('Q %d Rotor Speeds', i), 'FontWeight', 'bold', 'FontSize', 18, 'FontName', 'Times New Roman');
    ylabel('\omega (RPM)', 'FontWeight', 'bold', 'FontSize', 16, 'FontName', 'Times New Roman');
    ...
    xlim([0 60]);
    grid off; % Diğer grafiklerle uyum için grid kapalı, isterseniz 'on' yapın
    
    set(gca, 'FontName', 'Times New Roman', 'FontSize', 16, 'FontWeight', 'bold', 'LineWidth', 1.5, 'Layer', 'top');
    
    % Legend (Sadece ilk grafiğe veya hepsine eklenebilir, burada hepsine ekliyoruz ama 'best' konumunda)
    if has_rpm_data
        legend([p1 p2 p3 p4], 'Location', 'NorthEast', 'FontSize', 10, 'NumColumns', 2, 'Box', 'on');
    end
end

% KAYDETME
eps_name = fullfile(eps_folder, '17_rotor_rpms.eps');
exportgraphics(fig, eps_name, 'ContentType', 'vector');
exportgraphics(fig, output_pdf, 'Append', true);
close(fig);
fprintf('Rotor RPM grafikleri oluşturuldu ve eklendi.\n');